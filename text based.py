...
while True: # main loop
    ...
    print("\nType 'Add' or 'Remove' or 'Edit' and ENTER") # options
    inputoption = input("|>  ") # input

    if inputoption.lower() in ["+","add"]: # if add option selected
        add()

    if inputoption.lower() in ["-","remove","delete"]: # if remove option selected
        remove()

    if inputoption.lower() in [".","edit","change"]: # if edit option selected
        edit()
